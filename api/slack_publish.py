...
async def publish_welcomepage_to_slack(
    ...
    # Validate page completeness before allowing Slack publish
    from utils.page_validation import validate_page_completeness
    is_valid, validation_errors = validate_page_completeness(user, context="slack")
    
    if not is_valid:
        error_message = "Cannot publish incomplete page to Slack. Please add: " + ", ".join(validation_errors) + "."
        log.warning(f"Slack publish validation failed for user {request.user_public_id}: {error_message}")
        raise HTTPException(
            status_code=400,
            detail={
    ...
    # Check if user's page is already published
    # If already published, skip charging (payment already happened when publish button was clicked)
    if not user.is_draft:
        log.info(f"User {user.public_id} page is already published, skipping charge")
    else:
        ...
        published_count = db.query(WelcomepageUser).filter(
            ...
            WelcomepageUser.public_id != request.user_public_id  # Exclude current user
        ).count()
